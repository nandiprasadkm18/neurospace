...
export default function DreamWorkspacePage() {
  ...
  const { timerState, setTimer, tasks, toggleTask } = useWorkspaceStore();
  ...
  useEffect(() => {
    ...
    if (timerState.running && timerState.time > 0) {
      ...
      // Trigger sound or notification here
    }
        ...
        <DraggablePanel id="timer" title="Focus Timer" initialX={60} initialY={100}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              ...
              <button 
                onClick={() => setTimer({ time: 1500, running: false })}
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 24, justifyContent: 'center' }}>
              {['Pomodoro', 'Deep', 'Flow'].map(m => (
                <span key={m} style={{ fontSize: '0.6rem', fontFamily: 'var(--mono)', color: timerState.mode === m.toLowerCase() ? '#00FFFF' : 'rgba(255,255,255,0.3)', letterSpacing: '0.1em' }}>
                  {m.toUpperCase()}
                </span>
              ))}
            </div>
            ...
            {tasks.map(t => (
              <div 
                ...
                style={{ 
                  padding: '12px 16px', 
                  borderRadius: 12, 
                  background: t.completed ? 'rgba(0,255,136,0.05)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${t.completed ? 'rgba(0,255,136,0.2)' : 'rgba(255,255,255,0.08)'}`,
                  display: 'flex',
                  alignItems: 'center',
                  gap: 12,
                  cursor: 'pointer',
              ...
              >
                <div style={{ width: 14, height: 14, borderRadius: 3, border: `1px solid ${t.completed ? '#00FF88' : 'rgba(255,255,255,0.3)'}`, background: t.completed ? '#00FF88' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {t.completed && <div style={{ width: 6, height: 6, background: '#000', borderRadius: 1 }} />}
                </div>
                <span style={{ fontSize: '0.85rem', color: t.completed ? 'rgba(255,255,255,0.4)' : '#fff', textDecoration: t.completed ? 'line-through' : 'none', flex: 1 }}>
                  {t.text}
                </span>
                <span style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.2)', fontFamily: 'var(--mono)' }}>{t.category.toUpperCase()}</span>
              </div>
            ))}
            <button style={{ padding: '10px', borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: '0.8rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {['Deep Space', 'Rain', 'Forest', 'Café'].map(s => (
              <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <input type="range" style={{ width: 100, accentColor: '#00FFFF' }} />
        ...
        <DraggablePanel id="notes" title="Notes Archive" initialX={800} initialY={100}>
          <div style={{ minHeight: 300, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 12 }}>
              <span style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.2)', fontFamily: 'var(--mono)' }}>SAVED 2M AGO</span>
              <button style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: '#00FFFF', fontSize: '0.75rem', cursor: 'pointer', fontWeight: 600 }}>
